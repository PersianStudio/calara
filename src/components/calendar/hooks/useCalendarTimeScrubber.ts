import { PointerEvent as ReactPointerEvent, RefObject, useCallback, useEffect, useLayoutEffect, useState } from 'react';
import {
  MINUTES_PER_HOUR,
  clampMinutesToGrid,
  hourOfMinutes,
...
/**
 * Y inside a container of stacked hour rows → minutes.
 * Each direct child is one hour row; the fraction within a row maps to minutes.
 */
const measuredYToMinutes = (y: number, gridEl: HTMLElement, startHour: number, endHour: number) => {
...
const measuredMinutesToY = (minutes: number, gridEl: HTMLElement, startHour: number) => {
  const rows = Array.from(gridEl.children) as HTMLElement[];
...
/** Seed the scrubber at "now" when the viewed day/week includes today. */
export const scrubberInitialMinutes = (
  ...
  if (nowMinutes < startHour * MINUTES_PER_HOUR || nowMinutes > (endHour + 1) * MINUTES_PER_HOUR) return null;
  return nowMinutes;
};
