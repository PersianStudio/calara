...
const shiftDate = (view: DsCalendarView, date: Date, dir: -1 | 1) => {
  const m = moment(date);
  if (view === 'day') return m.add(dir, 'day').toDate();
  if (view === 'week') return m.add(dir, 'week').toDate();
  return m.add(dir, 'month').toDate();
};
  ...
  return (
    <div className="calara-toolbar">
      <div className="calara-toolbar__group">
        <button type="button" className="calara-btn" aria-label="Previous" onClick={() => onDateChange(shiftDate(view, currentDate, -1))}>
          ‹
        </button>
        <DsDatePicker
          variant="inlineText"
        ...
        />
        <button type="button" className="calara-btn" aria-label="Next" onClick={() => onDateChange(shiftDate(view, currentDate, 1))}>
          ›
        </button>
